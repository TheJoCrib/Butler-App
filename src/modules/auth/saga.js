...
import { loginWithEmail, signupWithEmail } from "./service";
import NavigationService from "../../utils/navigation";
import { signupStepsList } from "./config";
import { signInSuccess } from "./actions";
import { setAppLoading } from "../general/actions";
...
function* doLoginSuccess(token, userData = {}) {
  if (!userData?.isPhoneVerified) {
    ...
    yield call(
      ...
      signupStepsList[userData?.userType == "freelancer" ? 3 : 4]
    );
    yield put({
      type: Actions.SIGN_UP_PROCESSS_NAVIGATE_SUCCESS,
      payload: {
        signupStep: userData?.userType == "freelancer" ? 3 : 4,
        token: token,
        signUpProcessData: { userType: userData?.userType },
      },
    });
...
export default function* authSaga() {
  yield takeLatest(Actions.SIGN_IN_WITH_EMAIL, signInWithEmailSaga);
